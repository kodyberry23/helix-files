...
- **Local patch: sidebar diagnostics** - on the same socket helix pushes a complete diagnostics snapshot (`diagnostics-begin <sender> <seq>`, one `diagnostics <errors> <warnings> <path>` line per file that has any, `diagnostics-end <seq>`) whenever any file's LSP error/warning counts change, covering every file its language servers report on (open in a buffer or not; rust-analyzer and tsserver report on the whole crate/project). treelix 0.1.20+ applies a snapshot atomically, ignores one that is not newer than the last applied from the same sender (a restarted editor is a new sender and is applied at once), colors the file name red or yellow, shows the count after the name, and colors every folder above it by the worst severity inside, so a broken file in a collapsed folder is still visible. Errors and warnings only (hints and info stay quiet); `diagnostics = "errors"` or `"off"` in `treelix/config.toml` narrows it. Helix fingerprints its diagnostics map once per render (the same self-healing poll the follow patch uses, no allocation when nothing changed), writes from one sender thread in order, and resends the latest snapshot with every follow-reveal, so a sidebar restarted later catches up at the next buffer switch; the first snapshot goes out even when empty, so a fresh editor clears whatever its predecessor left. On exit helix flushes any snapshot still queued and appends `diagnostics-bye <sender> <seq>` to it on the same connection. treelix 0.1.23+ treats the farewell as an empty snapshot and applies it only if that editor's snapshot is the one on display, keeping the origin at the new sequence so that a snapshot of its own still in flight on another connection is treated as stale. An error fixed right before `:wq`, which rust-analyzer had no time to report, therefore does not stay red after the editor is gone. The wait for the farewell is bounded (250 ms), so a wedged sidebar cannot delay quitting. When two editors push to one sidebar, the most recent sender's set is shown, and one leaving does not wipe the other's markers.
...
- **Local patch: external edits re-check** - rust-analyzer runs cargo check (where most of its errors come from) only on `textDocument/didSave`, and deliberately ignores watched-file changes inside its workspace, expecting them to arrive as saves from the editor. A file fixed by another program (Claude Code, `git checkout`, a formatter) never produces one, so the stale error stayed in the gutter, the diagnostics picker and the sidebar until the next save of anything. Two additions: a buffer reload (auto-reload, `:reload`, `:reload-all`) now also notifies the file's language servers as a save, and a changed file no buffer holds is sent to rust-analyzer as its own `rust-analyzer/runFlycheck` notification (only files whose language lists the server, so cargo's own writes cannot start a check that starts another). Other servers already re-diagnose from `didChangeWatchedFiles`. Scenario 8 of `scripts/tests/test_treelix_diagnostics.py` covers it end to end.
- **[PR #13133](https://github.com/helix-editor/helix/pull/13133)** - inline git blame (closes [issue #3035](https://github.com/helix-editor/helix/issues/3035)). Virtual text after the cursor line with author, age, commit title and short hash, plus `space B` to print the line's blame in the statusline. Still open upstream and conflicting with master, so the merged copy is the community rebase at [NSPC911-forks/helix `gix-blame`](https://github.com/NSPC911-forks/helix/tree/gix-blame) (`c5f108276`, 2026-07-11), which already threads the workspace-trust level through blame requests. Local fixes on top of the fork: the `FileBlame` re-export is gated on the `git` feature (the PR breaks every `helix-vcs` build without it, e.g. `cargo test -p helix-vcs`); `helix-view` enables `helix-vcs/git` explicitly since `Document` stores blame state; the blame handler hooks upstream's `ConfigDidChange` instead of the PR's own event, which fired before the new config was stored so `:set inline-blame.auto-fetch true` never fetched anything; blame runs on tokio's blocking pool, capped at half the cores, and is sequenced per document (one at a time, results land in request order, a request that arrives mid-computation waits in a single slot that only the newest request occupies, keeping any line the user asked for) instead of synchronously inside the async hook, so no computation is thrown away and nothing older can overwrite something newer; a blame records the HEAD it was computed against and every request source goes through one `request_blame` that keeps a blame still computed against the current HEAD (the refs watcher also fires for a fetch, tag or stash) and leaves a blame already on its way alone, while a result that lands after HEAD moved on never displaces a current one (it is dropped, or kept only until the newer request already on its way lands, or replaced by a fresh request), a failed blame never displaces a usable one, never hides a current one and is never retried from its own failure, and a `space B` that had to fetch first is always answered, whichever of those happens, with the most recent press winning; the queue's state changes on the main thread as each result lands (one HEAD read judges both the result and what the document holds, a waiting request inherits any line still unanswered, and a closed buffer's queued work is dropped), so what it reports as in flight always matches what the editor holds; the refs watcher re-requests blame when HEAD moves (commit, amend, reset, checkout, deleted refs included), so external git operations update the virtual text without touching the buffer, and the paths that move on a commit (`HEAD`, the current branch ref, `packed-refs`) are polled in canonical form when they lie outside the workspace and re-resolved after every VCS change without resetting the baselines of paths already tracked (a path new to a re-resolved set fires once, so nothing that happened during the re-resolution is lost), so a linked worktree (including one of a bare main repository) or a submodule checkout opened as the workspace sees commits made on its branch (editing a submodule's files from the superproject is not tracked, for the gutter either) (`git replace` is not honoured: gix never applies object replacements, so neither the blame nor the diff gutter see them); realpath and repository opening share one prologue with the diff-base code (git records linked worktrees as realpaths, and macOS `/tmp` is a symlink); `{title}` uses git's `%s` summary instead of the raw title, which carried a trailing newline for commits without a body; and `{time-ago}` says "0 seconds ago" rather than "from now" for a commit made in the current second. `scripts/tests/test_inline_blame.py` drives the installed `hx` through a pty against a throwaway repo and checks all of this end to end (`cargo test` covers the blame engine and the handler's coalescing, not the editor loop). The `[editor.inline-blame]` block in `helix/config.toml` configures it; the `ui.virtual.inline-blame` scope in each theme styles it.
...
- `<CR>` on a file → treelix runs `scripts/dispatch-to-editor.sh open <path>`, which sends `:open-pick <path>` to the running helix over its Unix socket (helix-editor/helix PR #13896, in `local-patches`), then resolves the editor pane by its layout name and shifts zellij focus there. If nothing answers on the socket (the editor pane was closed, or helix quit), it spawns a replacement editor pane through `scripts/launch-editor.sh`, the same entry point the layout uses, so the new pane binds the session's helix socket and gets the treelix socket: the next pick routes into it rather than opening a third pane, and the sidebar follows it. Scenario 5 of `scripts/tests/test_zellij_config.py` covers this.
...
Colors in the sidebar, in precedence order: a file with LSP errors is red, with warnings yellow (and its folders with it; see "sidebar diagnostics" above); otherwise the git state colors the name (staged green, modified yellow, untracked orange, deleted/conflict red) with a glyph after the name; otherwise the plain text color. Executable scripts are no longer green: that collided with "staged" and made every `chmod +x` script look staged forever. Git status is rescanned after every filesystem change including `.git` itself (commit, push, checkout), one scan at a time so a scan started mid-commit can never land after, and overwrite, the one that saw the clean result; a failed scan is retried with backoff instead of leaving stale colors. `scripts/tests/test_treelix_diagnostics.py` drives the installed treelix and (with rust-analyzer present) the whole `hx` → sidebar pipeline through ptys.
...
| Key | Mode |
|---|---|
| `Ctrl p` | pane (h/j/k/l focus, n/d/r split, x close, f fullscreen, `Ctrl z` cycle pane frames, c rename, w toggle floating) |
...
Pane frames: `Ctrl p` then `Ctrl z` cycles them, full -> titles only -> none -> full (handy when copying with the mouse; zellij 0.45 made this a three-way cycle, and its new default style is titles only, so `zellij/config.kdl` pins `pane_frame_style "full"` or a fresh session looks frameless). Deliberately not on a direct Alt key, and not on zellij's default bare `z` either: helix uses Ctrl-p as "previous entry" in pickers, prompts and the completion menu, but zellij owns Ctrl-p in normal mode and silently enters pane mode with it (there is no status bar to show the switch), so the next `z` for helix's view mode was hiding every frame for the rest of the session. `scripts/tests/test_zellij_config.py` drives a throwaway session through a pty to keep both facts true.
