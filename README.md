...
- **Local patch: sidebar diagnostics** - on the same socket helix pushes `diagnostics <errors> <warnings> <path>` whenever a file's LSP error/warning counts change, for every file its language servers report on (open in a buffer or not; rust-analyzer and tsserver report on the whole crate/project). treelix 0.1.20+ colors the file name red or yellow, shows the count after the name, and colors every folder above it by the worst severity inside, so a broken file in a collapsed folder is still visible. Errors and warnings only (hints and info stay quiet); `diagnostics = "errors"` or `"off"` in `treelix/config.toml` narrows it. The push is computed from helix's diagnostics map once per render (the same self-healing poll the follow patch uses) and resent in full when the sidebar first answers.
