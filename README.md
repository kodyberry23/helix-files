...
- **Local patch: sidebar diagnostics** - on the same socket helix pushes a complete diagnostics snapshot (`diagnostics-begin <seq>`, one `diagnostics <errors> <warnings> <path>` line per file that has any, `diagnostics-end <seq>`) whenever any file's LSP error/warning counts change, covering every file its language servers report on (open in a buffer or not; rust-analyzer and tsserver report on the whole crate/project). treelix 0.1.20+ applies a snapshot atomically, ignores one older than the last applied, colors the file name red or yellow, shows the count after the name, and colors every folder above it by the worst severity inside, so a broken file in a collapsed folder is still visible. Errors and warnings only (hints and info stay quiet); `diagnostics = "errors"` or `"off"` in `treelix/config.toml` narrows it. Helix fingerprints its diagnostics map once per render (the same self-healing poll the follow patch uses, no allocation when nothing changed), writes from one sender thread in order, and resends the latest snapshot with every follow-reveal, so a sidebar restarted later catches up at the next buffer switch.
