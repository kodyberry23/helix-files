...
5c. Installs **zellij** pinned to a known-good version (`ZELLIJ_PINNED_VERSION` in `scripts/lib/common.sh`) via `cargo install zellij --version <pin> --locked` → `~/.cargo/bin/zellij` — **not** Homebrew. Pinned to `0.44.3`: it fixes an intermittent freeze on first attach to a new session (zellij [#5163](https://github.com/zellij-org/zellij/pull/5163) stale-render race, plus the #5152 deadlock fix). An earlier note blamed 0.44.3 for breaking Ghostty transparency (PR #4992/#5011); that was refuted empirically 2026-07-13 — both PRs already shipped in 0.44.1, and raw pty captures show 0.44.2 and 0.44.3 emit byte-identical background SGRs under this config. `setup.sh`/`update.sh` remove any brew-managed `zellij` so a later `brew upgrade` can't drift the version; `update.sh` re-pins if the installed version drifts. Bump the pin only after checking the changelog for rendering changes and re-confirming transparency.
